...
int main()
{
    ...
    XGpio adc_input[16], adc_output, adc_data_valid_input, adc_data_valid_output, adc_data_last_input, adc_data_last_output; // axi inputs and outputs
    ...
    XGpio_SetDataDirection(&adc_output, 1, 0); // declares the output direction of adc_output
    ...
    while (1)
    {
    	for (int i = 0; i < 16; i++)
    	{
    		temp[i] = XGpio_DiscreteRead(adc_input, 1);
    		XGpio_DiscreteWrite(&adc_output, 1, temp[i]); // prints ADC data one bit at a time
    	}
